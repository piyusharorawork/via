...
export const getVideoManagementMachine = (fetch: any) => {
  ...
  const videoManagementMachine = setup({
    types: {
      context: {} as { videos: ListVideosOutput; errorMessage: string | null },
      events: {} as
        | { type: "GET_VIDEOS" }
        | { type: "OPEN_ADD_VIDEO_FORM" }
        | { type: "CLOSE_ADD_VIDEO_FORM" }
        | { type: "ADD_VIDEO"; input: AddVideoInput },
    },
    actors: {
    ...
    states: {
      IDLE: {
        on: {
          GET_VIDEOS: "GETTING_VIDEOS",
          OPEN_ADD_VIDEO_FORM: "ADD_VIDEO_FORM_OPENED",
        },
      },
      GETTING_VIDEOS: {
        invoke: {
