...
export const getVideoManagementMachine = (fetch: any) => {
  ...
  const videoManagementMachine = setup({
    ...
    actors: {
      ...
      addVideo: fromPromise<AddVideoOutput, AddVideoInput>(
        async ({ input }) => {
          const video = await trpc.addVideo.mutate(input);
          return video;
        }
      ),
    ...
    actions: {
      ...
      saveVideoId: assign({
        addedVideoId: (data: any) => data.event.output,
      }),
      removeVideo: assign({
        videos: ({ context }) => {
      ...
      saveAddVideoId: assign({
        addedVideoId: (data: any) => {
          return data.event.output.videoUUID;
        },
      }),
    },
  }).createMachine({
    ...
    context: ({ input }) => {
      return {
        ...
        addedVideoId: "", // video id of recently added video
      };
    },
    initial: "VideosPage",
    states: {
        ...
        states: {
          ...
          addingVideo: {
            invoke: {
              ...
              input: (data: any) => data.event.input,
              onDone: {
                target: "addingVideoSuccess",
                actions: "saveAddVideoId",
              },
          ...
          //TODO may be just send events to the videos machine to select the video
          selectingVideo: {
            invoke: {
              src: "viewVideo",
              input: ({ context }) => ({ videoUUID: context.addedVideoId }),
              onDone: { target: "fetchingVideos", actions: "saveVideoDetails" },
              onError: "#VideosPage.loadingVideoDetailsFailed",
            },
          },
          // Instead of fetching may be just add the video to list
          fetchingVideos: {
            invoke: {
              src: "listVideos",
              onDone: {
